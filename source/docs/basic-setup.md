...
class Config
{
     ...
     * 
     * @see Documentation: docs/configuration/config-overview
     */
...
For different environments (development, staging, production), you can use environment-specific configurations:
...
Configure security settings in your `Security.php` file:
...
class Security
{
    ...
    {
        // Add security hooks
        add_action('init', [self::class, 'setup_security']);
    }
    ...
    public static function setup_security()
    {
        ...
        add_action('wp_head', [self::class, 'add_security_headers']);
    }
